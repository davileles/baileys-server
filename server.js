...
app.get('/buscar-produto', async (req, res) => {
  ...
  try {
    ...
    // 2. Extrai só o texto relevante do HTML (remove scripts, styles, tags)
    const textoLimpo = html
      .replace(/<script[\s\S]*?<\/script>/gi, '')
      .replace(/<style[\s\S]*?<\/style>/gi, '')
      .replace(/<[^>]+>/g, ' ')
      .replace(/\s{2,}/g, ' ')
      .substring(0, 8000); // limita para não estourar o contexto

    // 3. Manda para a Claude API extrair nome e preço
    const claudeResp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `Extraia APENAS o nome do produto e o preço do texto abaixo, que veio de uma página de e-commerce.
