...
// Buscar nome e preço de produto por URL
app.get('/buscar-produto', async (req, res) => {
  const { url } = req.query;
  if (!url) return res.status(400).json({ ok: false, erro: 'Parâmetro "url" obrigatório.' });
  ...
  try {
    ...
    if (!resp.ok) return res.status(502).json({ ok: false, erro: `Site retornou ${resp.status}` });
    ...
    if (finalUrl.includes('amazon.com')) {
      const nomeMatch = html.match(/<span[^>]*id=["']productTitle["'][^>]*>([\s\S]*?)<\/span>/i);
      if (nomeMatch) nome = nomeMatch[1].trim();

      const precoMatch =
        html.match(/<span[^>]*class=["'][^"']*a-price-whole["'][^>]*>([\d.,]+)/i) ||
        html.match(/"priceAmount"\s*:\s*([\d.]+)/i);
      if (precoMatch) preco = precoMatch[1].replace(',', '').trim();
    }

    // ── MERCADO LIVRE ─────────────────────────────────────────
    else if (finalUrl.includes('mercadolivre.com') || finalUrl.includes('mercadolibre.com') || finalUrl.includes('ml.bz') || finalUrl.includes('mlb.me')) {
      const nomeMatch =
        html.match(/<h1[^>]*class=["'][^"']*ui-pdp-title["'][^>]*>([\s\S]*?)<\/h1>/i) ||
        html.match(/<meta[^>]*property=["']og:title["'][^>]*content=["']([^"']+)["']/i);
      if (nomeMatch) nome = nomeMatch[1].replace(/<[^>]+>/g, '').trim();

      const precoMatch =
        html.match(/<span[^>]*class=["'][^"']*andes-money-amount__fraction["'][^>]*>([\d.,]+)/i) ||
        html.match(/"price"\s*:\s*([\d.]+)/i);
      if (precoMatch) preco = precoMatch[1].replace(/\./g, '').replace(',', '.').trim();
    }

    // ── SHOPEE ────────────────────────────────────────────────
    else if (finalUrl.includes('shopee.com')) {
      const nomeMatch = html.match(/<meta[^>]*property=["']og:title["'][^>]*content=["']([^"']+)["']/i);
      if (nomeMatch) nome = nomeMatch[1].trim();

      const precoMatch = html.match(/"price_before_discount"\s*:\s*(\d+)/i) ||
        html.match(/"price"\s*:\s*(\d+)/i);
      if (precoMatch) {
        const raw = parseInt(precoMatch[1]);
        preco = (raw / 100000).toFixed(2);
      }
    }
    ...
    if (!nome) {
      ...
      if (og) nome = og[1].trim();
        ...
        if (title) nome = title[1].replace(/\s*[-|].*$/, '').trim();
      }
    }

    if (!nome && !preco)
      return res.json({ ok: false, erro: 'Não foi possível extrair dados desta página.' });

    res.json({ ok: true, nome: nome || null, preco: preco || null });
  } catch (err) {
    console.error('Erro ao buscar produto:', err.message);
    res.status(500).json({ ok: false, erro: 'Erro ao acessar a URL: ' + err.message });
  }
});
