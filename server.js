...
function mesclarParesIdaVolta(validas) {
  ...
  for (let i = 0; i < validas.length; i++) {
    ...
    for (let j = i + 1; j < validas.length; j++) {
      ...
      const mesmoPrograma = (v.programa||'') === (w.programa||'');
      const mesmaCabine   = (v.cabine||'Economica') === (w.cabine||'Economica');
      // Normaliza: prefere código IATA, senão usa cidade resolvida (minúsculo para evitar case mismatch)
      const vOri = (v.origemCodigo  || resolverCidade('', v.origem)  || '').toLowerCase();
      const vDes = (v.destinoCodigo || resolverCidade('', v.destino) || '').toLowerCase();
      const wOri = (w.origemCodigo  || resolverCidade('', w.origem)  || '').toLowerCase();
      const wDes = (w.destinoCodigo || resolverCidade('', w.destino) || '').toLowerCase();
      const rotaInvertida = vOri && vDes && wOri && wDes
                         && vOri === wDes && vDes === wOri;
    ...
    if (parIdx !== -1) {
      ...
      // Reaplica resolverCidade para garantir nome correto após merge
      merged.origem  = resolverCidade(merged.origemCodigo,  merged.origem);
      merged.destino = resolverCidade(merged.destinoCodigo, merged.destino);
